...
const PROMPT_TEMPLATES: Record<string, PromptParts> = {
  ...
  'pd_confirm': {
    ...
    user: `
        ...
        ## nodes: "  # <instruction>"
        - Implementation approach: # 不要输出内容

        - File list:  # 生成所有后端代码文件,不包括前端,后端java程序入口文件(Application.java),后端配置文件(pom.xml,MyBatisConfig.java),sql文件。
        Only need relative paths.只包括文件,不要写其他东西

        - Flow chart Diagram: # 不要输出内容
        ...
        - frontEnd clientApi: # 不要输出内容。

        - frontEnd flow chart: # 不要输出内容

        - Total File list:首先, 必须实现"Data structures and interfaces"的类, 不能遗漏。每个类由一个文件实现,不要一个文件实现全部类,例如有class A,class B,则A.java实现class A, B.java实现class B,文件名是类名。
        其次, 参考"Program call flow"的内容, 检查File list是否有遗漏的文件, 如有将补充的文件和原来File list合并后一起展示, 仅补充必要的核心文件, 不要新增不重要的文件, 例如异常处理, 布局等非核心文件。
        Only need relative paths.只包括文件,不要写其他东西

        - Anything UNCLEAR:  # Mention unclear project aspects, then try to clarify it.
        - checklist: 检查设计文档问题
        1.将Requirement Pool和相关类图关联,如"需求1(P0) ---- class A, class B",检查是否有遗漏生成的类图。需求1只是举例,换成具体需求。
  ...
  'arch_adjust': {
    ...
    user: `
        ...
        - File list: typing.List[str]  # # 生成所有后端代码,不要包括前端代码,包括后端java程序入口文件(Application.java),后端配置文件(pom.xml,MyBatisConfig.java),sql文件。
        Only need relative paths.只包括文件,不要写其他东西
        ...
        - Total File list:首先, 必须实现"Data structures and interfaces"中定义的类, 不能遗漏。每个类由一个文件实现,不要一个文件实现全部类,例如有class A,class B,则A.java实现class A, B.java实现class B,文件名是类名。
        其次, 参考"Program call flow"的内容, 检查File list是否有遗漏的文件, 如有将补充的文件和原来File list合并后一起展示, 仅补充必要的核心文件, 不要新增不重要的文件, 例如异常处理, 布局等非核心文件。
        Only need relative paths.只包括文件,不要写其他东西
